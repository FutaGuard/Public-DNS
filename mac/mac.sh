...
curl -o "./dnsproxy-darwin-${arch}-${version}.tar.gz" -fL "https://github.com/AdguardTeam/dnsproxy/releases/download/${version}/dnsproxy-darwin-${arch}-${version}.tar.gz"

echo -e "${GREEN}[info]${NC} 解壓縮..."
tar zxvf "dnsproxy-darwin-${arch}-${version}.tar.gz"

if [ ! -f "com.adguardteam.dnsproxy.plist" ]; then
...
# 壞壞的操作。
# sudo -s
sudo cp ./darwin-${arch}/dnsproxy /usr/local/bin/
sudo cp ./com.adguardteam.dnsproxy.plist /Library/LaunchDaemons
if [ -f "/Library/LaunchDaemons/com.adguardteam.dnsproxy.plist" ]; then
  sudo launchctl unload /Library/LaunchDaemons/com.adguardteam.dnsproxy.plist
fi
sudo launchctl load -w /Library/LaunchDaemons/com.adguardteam.dnsproxy.plist

# 自己的大便自己掃
rm -rf darwin-${arch} dnsproxy-darwin-${arch}-${version}.tar.gz